...
export const buildMessage = (inputProps: Data,) => {
  if (inputProps.minute < 10 && inputProps.awayScore == 0) {
    return replace(earlyWith0Message[randomNumber(0, earlyWith0Message.length - 1)], inputProps)
  }
  if (inputProps.awayScore === inputProps.homeScore) {
    return replace(equalMessage[randomNumber(0, randomNumber.length - 1)], inputProps)
  }
  return replace(basicMessage[randomNumber(0, basicMessage.length - 1)], inputProps)
}
